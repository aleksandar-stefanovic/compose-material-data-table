...
@Composable
internal fun <T> DoubleFilterModal(
    columnSpec: DoubleColumnSpec<T>,
    onFilterConfirm: (NumberFilter<T, Double>) -> Unit,
    onClose: () -> Unit
) {
    ...
    val onConfirm = {
        if (canConfirm) {
            onFilterConfirm(NumberFilter(columnSpec, selectedPredicate, listOf(firstValue!!, secondValue ?: 0.0)))
        }
    }
